...
export const useValidate = (form = {}, setStateErrors = null) => {
    ...
    for (const dataObj in form) {
        ...
        if (!isErrorObj) {
            forms[dataObj] = form?.[dataObj] ?? '';
        }
    }
    const result = validate(forms, Constraints);
    for (const resForms in forms) {
        errors[`${resForms}Error`] = '';
        for (const resErr in result) {
            if (resErr === resForms) {
                errors[`${resForms}Error`] = !isEmpty(result?.[resErr]?.[0])
                    ? result?.[resErr]?.[0]
                    : '';
            }
        }
    ...
    if (typeof setStateErrors === 'function') {
        setStateErrors(state => ({ ...state, ...errors }));
    }
